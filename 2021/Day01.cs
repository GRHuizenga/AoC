...
namespace _2021
{
    public class Day01 : Day<int>
    {
        ...
        {
            var tuples = InputLines.Zip(InputLines.Skip(1), (left, right) => Tuple.Create(int.Parse(left), int.Parse(right)));
            return tuples.Count(tuple => tuple.Item2 > tuple.Item1);
        }

        public override int PartTwo()
        {
            var sums = InputLines.Select(measurement => int.Parse(measurement)).SlidingWindow(3).Select(triplet => triplet.Sum());
